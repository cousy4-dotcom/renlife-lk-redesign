import { CalendarDays, CheckCircle2, ChevronRight, CreditCard, FileText, HeartHandshake, MessageSquareText, WalletCards } from 'lucide-react';
...
import { contracts, documents, notifications } from '@/data/mock';
...
const dashboardActions = [
  { label: 'Оплатить взнос', icon: CreditCard },
  { label: 'Скачать полис', icon: FileText },
  { label: 'Связаться с менеджером', icon: HeartHandshake },
  { label: 'Подать заявление', icon: MessageSquareText },
];
  ...
  return (
    <AppShell>
      <div className="space-y-3 md:space-y-4">
        ...
        <section className="grid gap-2.5 md:gap-4 lg:grid-cols-2">
          <Card>
          ...
          <Card>
            ...
            <div className="grid gap-2">{dashboardActions.map(({ label, icon: Icon }) => <button key={label} className="flex min-h-10 items-center justify-between rounded-2xl bg-lavender px-3 text-left text-sm font-bold text-brand-900"><span className="flex items-center gap-3"><Icon size={18} />{label}</span><ChevronRight size={16} /></button>)}</div>
          </Card>
        </section>
      </div>
    </AppShell>
