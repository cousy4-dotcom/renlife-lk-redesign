...
import { client, contracts, documents, financialChart, financialProgress, notifications } from '@/data/mock';
...
const moneyMetrics = [
  ...
  { label: 'Очередной взнос', value: financialProgress.nextPayment, hint: `Ближайшая дата — ${financialProgress.nextPaymentDate}.` },
];
...
const quickActions = [
  ...
  { label: 'Связаться с менеджером', icon: MessageCircle, href: '/profile' },
];
  ...
  return (
    <AppShell>
      <div className="space-y-5 pb-4 md:space-y-6 lg:pb-0">
          ...
          <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_340px]">
            ...
            <div className="flex flex-col justify-between bg-brand-900 p-5 text-white md:p-7">
              <div>
                ...
                <p className="mt-5 text-sm leading-6 text-white/70">Договор активен. Ближайший взнос — {financialProgress.nextPayment} до {financialProgress.nextPaymentDate}, документы и история платежей доступны ниже.</p>
              </div>
        ...
        <Card className="border border-amber-100 bg-white">
          <div className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-center">
            <div className="flex gap-4">
              ...
              <div><div className="flex flex-wrap items-center gap-2"><h2 className="text-2xl font-black text-brand-900">Что важно сейчас</h2><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-extrabold text-amber-700">Требует внимания</span></div><p className="mt-3 text-xl font-black text-brand-900 md:text-2xl">Следующий взнос {financialProgress.nextPayment} до {financialProgress.nextPaymentDate}</p><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">Оплатите взнос вовремя, чтобы страховая защита продолжала действовать без перерыва.</p></div>
            </div>
            <Button href={paymentHref} className="w-full lg:w-auto">Оплатить</Button>
          </div>
        </Card>

        <section><div className="mb-3 flex items-center gap-3"><WalletCards className="text-brand-900" /><h2 className="text-2xl font-black text-brand-900">Параметры договора</h2></div><div className="grid gap-3 md:grid-cols-2">{moneyMetrics.map((item) => <Card key={item.label} className="p-5"><p className="text-sm font-bold text-slate-400">{item.label}</p><p className="mt-2 text-2xl font-black text-brand-900 md:text-3xl">{item.value}</p><p className="mt-3 text-sm leading-5 text-slate-500">{item.hint}</p></Card>)}</div></section>

        <FinancialProgress data={financialProgress} variant="expanded" />

        <SavingsChart points={financialChart} />
        ...
        <Card><h2 className="text-2xl font-black text-brand-900">Последние события по договору</h2><div className="mt-4 grid gap-3">{notifications.slice(0, 3).map((event) => { const Icon = event.icon; return <div key={event.id} className="flex gap-3 rounded-[1.2rem] bg-lavender p-4"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-[.9rem] bg-white text-brand-900"><Icon size={18} /></div><div><p className="font-black text-brand-900">{event.title}</p><p className="mt-1 text-sm text-slate-500">{event.date} · {event.text}</p></div></div>; })}</div></Card>
